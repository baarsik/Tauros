// dear imgui, v1.50 WIP, edited by Барс
// (main code)

// See ImGui::ShowTestWindow() in imgui_demo.cpp for demo code.
// Get latest original version at https://github.com/ocornut/imgui
// Original releases change-log at https://github.com/ocornut/imgui/releases
...
void ImGuiIO::AddInputCharactersUTF8(const char* utf8_chars)
{
    ...
    for (auto i = 0; i < wchars_buf_len && wchars[i] != 0; i++)
...
{
    if (count < 1) return;
    strncpy(dst, src, static_cast<size_t>(count));
    dst[count-1] = 0;
}
...
char* ImStrdup(const char *str)
{
    ...
    return static_cast<char*>(memcpy(buff, static_cast<const void*>(str), len));
}
...
const char* ImStristr(const char* haystack, const char* haystack_end, const char* needle, const char* needle_end)
{
    ...
    const char un0 = static_cast<char>(toupper(*needle));
...
{
    IM_ASSERT(buf_size > 0);
	auto w = vsnprintf(buf, buf_size, fmt, args);
    if (w == -1 || w >= buf_size)
        w = buf_size - 1;
    buf[w] = 0;
    return w;
...
int ImTextCharFromUtf8(unsigned int* out_char, const char* in_text, const char* in_text_end)
{
	...
	auto str = reinterpret_cast<const unsigned char*>(in_text);
    if (!(*str & 0x80))
    {
    ...
    if ((*str & 0xe0) == 0xc0)
    {
        *out_char = 0xFFFD; // will be invalid but not end of string
        if (in_text_end && in_text_end - reinterpret_cast<const char*>(str) < 2) return 1;
    ...
    {
        *out_char = 0xFFFD; // will be invalid but not end of string
        if (in_text_end && in_text_end - reinterpret_cast<const char*>(str) < 3) return 1;
    ...
    {
        *out_char = 0xFFFD; // will be invalid but not end of string
        if (in_text_end && in_text_end - reinterpret_cast<const char*>(str) < 4) return 1;
...
bool ImGui::SliderBehavior(const ImRect& frame_bb, ImGuiID id, float* v, float v_min, float v_max, float power, int decimal_precision, ImGuiSliderFlags flags)
{
    ...
    if (is_horizontal)
		...
		grab_bb = ImRect(ImVec2(frame_bb.Min.x, frame_bb.Min.y + grab_padding), ImVec2(grab_pos + grab_sz*0.5f, frame_bb.Max.y - grab_padding));
    else
...
bool ImGui::Checkbox(const char* label, bool* v)
{
	...
	auto check_bb_center = check_bb.GetCenter() - CalcTextSize(ICON_FA_SQUARE_O) / 2.0f;
    RenderText(check_bb_center, *v ? ICON_FA_CHECK_SQUARE_O : ICON_FA_SQUARE_O);
...
static bool Items_VectorGetter(void* vec, int idx, const char** out_text)
{
...
bool ImGui::Combo(const char* label, int* current_item, std::vector<std::string> items, int height_in_items)
{
	if (items.empty()) return false;
	auto value_changed = Combo(label, current_item, Items_VectorGetter, static_cast<void*>(&items), items.size(), height_in_items);
	return value_changed;
}
