...
public:
	...
	bool IsGrenade()
    {
		...
		return classId == EClassIds::CBaseCSGrenade || classId == EClassIds::CBaseGrenade ||
			classId == EClassIds::CHEGrenade || classId == EClassIds::CDecoyGrenade ||
			classId == EClassIds::CIncendiaryGrenade || classId == EClassIds::CMolotovGrenade ||
			classId == EClassIds::CSensorGrenade || classId == EClassIds::CSmokeGrenade ||
			classId == EClassIds::CFlashbang;
    }
	bool IsKnife()
    {
		auto classId = this->GetClientClass()->m_ClassID;
		return classId == EClassIds::CKnife || classId == EClassIds::CKnifeGG;
    }
	bool IsC4()
    {
		auto classId = this->GetClientClass()->m_ClassID;
		return classId == EClassIds::CC4;
    }
};
...
public:
	...
	bool IsObservingTarget(C_CSPlayer* pTarget)
    {
		...
		auto observeTarget = static_cast<C_CSPlayer*>(Interfaces::EntityList()->GetClientEntityFromHandle(m_hObserverTarget));
